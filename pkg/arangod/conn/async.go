...
func (a async) Do(ctx context.Context, req driver.Request) (driver.Response, error) {
	if id, ok := a.isAsyncIDSet(ctx); ok {
		...
		case http.StatusNoContent:
			asyncID := resp.Header(api.ArangoHeaderAsyncIDKey)
			if asyncID == id {
				// Job is done
				return resp, nil
			}
		...
		req.SetHeader(api.ArangoHeaderAsyncKey, api.ArangoHeaderAsyncValue)
		...
		case http.StatusAccepted:
			if v := resp.Header(api.ArangoHeaderAsyncIDKey); len(v) == 0 {
